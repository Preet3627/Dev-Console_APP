...
final class Dev_Console_Connector {
    ...
    private function _action_get_asset_files($payload) { $id = sanitize_text_field($payload['assetIdentifier']); $type = $payload['assetType']; $rel_path = isset($payload['path']) ? $payload['path'] : ''; $base_path_root = ''; if ($id === 'root') { $base_path_root = ABSPATH; } elseif ($type === 'plugin') { $base_path_root = WP_PLUGIN_DIR . '/' . dirname($id); } elseif ($type === 'theme') { $base_path_root = get_theme_root() . '/' . $id; } if (empty($base_path_root) || !is_dir($base_path_root)) throw new Exception("Asset directory not found."); $base_path_root = realpath($base_path_root); $scan_path = $this->get_validated_path(['assetIdentifier' => $id, 'assetType' => $type, 'relativePath' => $rel_path]); if (!is_dir($scan_path)) throw new Exception("Path is not a valid directory."); $files = []; $items = scandir($scan_path); foreach ($items as $item) { if ($item === '.' || $item === '..') continue; $full_item_path = $scan_path . DIRECTORY_SEPARATOR . $item; $item_rel_path = ltrim(str_replace($base_path_root, '', $full_item_path), DIRECTORY_SEPARATOR); $files[] = ['name' => $item, 'path' => $item_rel_path, 'type' => is_dir($full_item_path) ? 'directory' : 'file']; } return $files; }
