export const pluginSourceCode = [
  ...
  '}',
  '',
  '// FIX: Added checks to prevent "Cannot redeclare constant" fatal errors.',
  ...
  '        // Ensure pluggable.php is loaded for wp_generate_password, a common issue during activation hooks.',
  ...
  '        if (!WP_Filesystem()) { throw new Exception(\'Could not initialize WordPress Filesystem. Please check your wp-config.php for FS_METHOD.\'); }',
  '        $path = $this->resolve_path($payload[\'assetIdentifier\'], $payload[\'assetType\'], $payload[\'relativePath\']);',
  '        if (file_exists($path)) { $this->backup_file($path, $payload[\'assetIdentifier\'], $payload[\'assetType\'], $payload[\'relativePath\']); }',
  '        if (!$wp_filesystem->put_contents($path, $payload[\'content\'], FS_CHMOD_FILE)) { throw new Exception(\'Failed to write file. This could be a file permissions issue.\'); }',
  '        return [\'status\' => \'ok\'];',
  '    }',
  '',
  ...
  '            unlink($zip_path);',
  '            throw new Exception(\'Backup file exceeds 50MB and cannot be transferred directly. Please use a dedicated backup solution.\');',
  '        }',
  '',
  ...
  '',
  '        $path = __FILE__;',
  '        if (!file_exists($path)) { throw new Exception(\'Connector plugin file not found.\'); }',
  '        ',
  '        copy($path, $path . \'.bak\');',
  '        if (!$wp_filesystem->put_contents($path, $payload[\'content\'], FS_CHMOD_FILE)) {',
  '            copy($path . \'.bak\', $path); // Rollback',
  '            @unlink($path . \'.bak\');',
  '            throw new Exception(\'Failed to update plugin file. This is often a file permissions issue on your server.\');',
  '        }',
  '        @unlink($path . \'.bak\');',
