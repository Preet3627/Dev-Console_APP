...
const CodeEditor: React.FC<CodeEditorProps> = ({ siteData, asset, initialFile, onClose, modalBgColor }) => {
    ...
    return (
        <div className={`fixed inset-0 bg-overlay backdrop-blur-sm flex items-center justify-center z-50 ${isFullScreen ? '' : 'p-4'}`}>
            <div
                style={modalStyle}
                className={`bg-background-secondary shadow-2xl border border-border-primary flex flex-col transition-all duration-300 ${isFullScreen ? 'w-full h-full rounded-none' : 'rounded-lg w-full max-w-7xl h-[90vh]'} p-6`}
            >
